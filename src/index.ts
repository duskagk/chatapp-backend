...
io.on('connection', (socket) => {
  ...
  // 방 참가
  socket.on('join-room', async (data: { username: string; roomId?: string }) => {
    try {
      ...
      // 사용자 추가
      const user = userService.addUser(socket.id, {
        username,
        isOnline: true,
        joinedAt: new Date()
      ...
      io.to(roomId).emit('room-users', roomUsers.map(u => ({
        id: u.id,
        username: u.username,
        isOnline: u.isOnline,
        joinedAt: u.joinedAt
  ...
  socket.on('send-message', (messageData) => {
    try {
      ...
      console.log(`💬 Message from ${user.username} in ${user.currentRoom}: ${message.content}`);
  ...
  socket.on('disconnect', (reason) => {
    ...
    try {
      ...
      if (user && user.currentRoom) {
        ...
        socket.to(user.currentRoom).emit('room-users', roomUsers.map(u => ({
          id: u.id,
          username: u.username,
          isOnline: u.isOnline,
          joinedAt: u.joinedAt
...
setInterval(() => {
  ...
  if (stats.onlineUsers > 0) {
    console.log(`📊 Stats: ${stats.onlineUsers} online users in ${stats.totalRooms} rooms`);
  }
}, 60000); // 1분마다

// 비활성 사용자 정리 (선택사항)
setInterval(() => {
  const removedUsers = userService.cleanupInactiveUsers();
